...
@section('content')
    ...
    <div class="flex justify-center"> <!-- Centrar el contenedor principal -->
        <div class="max-w-2xl mx-auto py-8 px-6 rounded-lg shadow-md flex-grow" style="background-color: rgb(34, 37, 42);">
            <!-- Contenedor con mejor diseño -->
            <h1 class="text-3xl font-bold text-white mb-6">Registrar Horario</h1>

            <form id="buscarDoctorForm">
                @csrf
            ...
            <form id="scheduleForm" action="" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                <div class="p-4 bg-gray-700 rounded-lg space-y-4" style="background-color: rgb(46, 49, 54);">
                    <!-- Contenedor para el registro de horario -->
                    <div>
                        <label class="block">
                    ...
                    <div>
                        <label class="block">
                            ...
                            <input type="number" name="intervalo" id="intervalo" value="15" min="5"
                                max="60" step="5"
                                class="mt-1 block w-full rounded-md border-red-500 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 p-2">
                        </label>
            ...
            <div id="additionalSection" class="mt-6 p-4 rounded-lg text-white"
                style="background-color: rgb(46, 49, 54); display: none;">
                <button type="button" id="registerButton"
                    class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-500">Registrar</button>
            </div>
        </div>
    </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.getElementById('buscarDoctorForm').addEventListener('submit', function(e) {
                ...
                fetch('{{ route('buscardoctor') }}', {
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            document.getElementById('doctorName').value = data.nombre;
                            document.getElementById('doctorSpecialty').value = data.especialidad;
                        } else {
                            alert(data.message);
                            console.log('Mensaje de error del servidor:', data.message);
                        }
                    })
                    .catch(error => {
            ...
            document.getElementById('createSchedule').addEventListener('click', function() {
                ...
                for (let tiempo = inicioEnMinutos; tiempo < finEnMinutos; tiempo += intervalo) {
                    ...
                    const horasInicioStr = horasInicio >= 12 ? (horasInicio - 12 || 12) :
                    horasInicio; // Convertir a formato 12 horas
                    const amPmInicio = horasInicio >= 12 ? 'PM' : 'AM';
                    const horasFinStr = horasFin >= 12 ? (horasFin - 12 || 12) :
                    horasFin; // Convertir a formato 12 horas
                    ...
                    const intervaloStr =
                        `${horasInicioStr}:${minutosInicioStr} ${amPmInicio} - ${horasFinStr}:${minutosFinStr} ${amPmFin}`;
                    intervalos.push(intervaloStr);
                }

                // Crear el contenido para mostrar
                const fechaCita = new Date(fecha).toLocaleDateString('es-ES', {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric'
                });
            ...
            `;
                ...
                // Mostrar la sección adicional con el botón Registrar
                const additionalSection = document.getElementById('additionalSection');
                additionalSection.style.display = 'block';

                // Guardar los intervalos en el formulario para su envío
                document.getElementById('intervalos').value = JSON.stringify(intervalos);
            });
