...
@section('soprote')
    ...
    <div class="h-screen w-full bg-gradient-to-br from-gray-900 to-gray-800 p-6 overflow-hidden">
        <!-- Header Section -->
        <div class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-3xl font-bold text-white">Gestión de Especialidades Médicas</h1>
                <p class="text-gray-400">Administra las especialidades disponibles en el sistema</p>
            </div>
            <div class="flex items-center space-x-4">
                <form method="GET" action="{{ route('add.especialidades') }}" class="flex items-center space-x-2">
                    <div class="relative">
                        <input type="text" name="buscar" placeholder="Buscar especialidad..."
                               value="{{ request('buscar') }}"
                               class="pl-10 pr-4 py-2 rounded-full border border-gray-700 bg-gray-800 text-white focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent w-64">
                    ...
                    @if(request()->has('buscar'))
                        <a href="{{ route('add.especialidades') }}"
                           class="bg-gray-700 hover:bg-gray-600 text-gray-300 px-4 py-2 rounded-lg shadow-md transition-all duration-300">
                            Limpiar
                        </a>
                    @endif
                </form>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-12 gap-6 h-[calc(100%-120px)]">
            <!-- Add Speciality Card -->
            <div class="xl:col-span-4 bg-gray-800 rounded-2xl shadow-xl p-6 border border-gray-700 overflow-y-auto">
                @php
                    $editando = isset($especialidad);
                @endphp
        ...
        <span class="w-8 h-8 {{ $editando ? 'bg-red-900' : 'bg-blue-900' }} rounded-full flex items-center justify-center mr-3">
            <i class="fas {{ $editando ? 'fa-edit text-red-300' : 'fa-plus text-blue-300' }}"></i>
        </span>
                        {{ $editando ? 'Actualizando Especialidad' : 'Nueva Especialidad' }}
                    </h2>
                    <div class="{{ $editando ? 'bg-red-900 text-red-300' : 'bg-blue-900 text-blue-300' }} text-xs font-medium px-2.5 py-0.5 rounded-full">
                        {{ $editando ? 'Modo edición' : 'Paso 1 de 2' }}
                    </div>
                </div>
                ...
                <form method="POST"
                    ...
                    <div class="space-y-1">
                        <label class="block text-sm font-medium text-gray-300">Nombre de la especialidad</label>
                        <div class="relative">
                            <input type="text" name="nombre" placeholder="Ej: Pediatría"
                                   value="{{ $editando ? $especialidad->nombre : old('nombre') }}"
                                   class="w-full px-4 py-3 border border-gray-700 bg-gray-700 text-white rounded-lg focus:ring-2 focus:ring-blue-600 focus:border-transparent transition-all duration-200">
                            <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                    ...
                    <div class="space-y-1">
                        ...
                        <textarea name="descripcion" rows="4"
                                  placeholder="Área médica que atiende a niños y adolescentes..."
                                  class="w-full px-4 py-3 border border-gray-700 bg-gray-700 text-white rounded-lg focus:ring-2 focus:ring-blue-600 focus:border-transparent transition-all duration-200">{{ $editando ? $especialidad->descripcion : old('descripcion') }}</textarea>
                    </div>
                ...
                @if(Session::has('success'))
                    <div id="notification"
                         class="fixed top-24 right-6 w-80 bg-green-900 border border-green-700 text-green-300 px-4 py-3 rounded-lg shadow-lg flex items-center justify-between animate-slideIn">
                        <div>
                            <i class="fas fa-check-circle mr-2"></i>
                            {{ Session::get('success') }}
                        </div>
                        <button onclick="closeNotification()" class="text-green-300 hover:text-green-100">
                            <i class="fas fa-times"></i>
                        </button>
                    </div>
                @endif
                ...
                <div id="errorNotification"
                     class="hidden fixed top-36 right-6 w-80 bg-red-900 border border-red-700 text-red-300 px-4 py-3 rounded-lg shadow-lg flex items-center justify-between animate-slideIn">
                    <div id="errorMessage" class="flex items-center">
                        <i class="fas fa-exclamation-circle mr-2"></i>
                        <span></span>
                    </div>
                    <button onclick="closeErrorNotification()" class="text-red-300 hover:text-red-100">
                        <i class="fas fa-times"></i>
                    </button>
                </div>

                <div class="mt-8 pt-6 border-t border-gray-700">
                    <h3 class="text-sm font-medium text-gray-400 mb-3">Especialidades sugeridas</h3>
                    <div class="flex flex-wrap gap-2">
                        @php
                            $sugeridas = [
                                ['Pediatría', 'Especialidad médica dedicada a la salud infantil y adolescente'],
                        ...
                        @foreach ($sugeridas as [$nombre, $descripcion])
                            <span
                                class="suggested-specialty bg-gray-700 hover:bg-gray-600 text-gray-300 text-xs font-medium px-3 py-1.5 rounded-full cursor-pointer transition"
                                data-nombre="{{ $nombre }}"
                                data-descripcion="{{ $descripcion }}">
                {{ $nombre }}
            </span>
            ...
            <!-- Specialities Table -->
            <div class="xl:col-span-8 bg-gray-800 rounded-2xl shadow-xl p-6 border border-gray-700 overflow-hidden">
                <div class="flex flex-col h-full">
                    <div class="flex items-center justify-between mb-6">
                        <h2 class="text-xl font-semibold text-white flex items-center">
            <span class="w-8 h-8 bg-blue-900 rounded-full flex items-center justify-center mr-3">
                <i class="fas fa-list text-blue-300"></i>
            </span>
                            Especialidades Registradas
                        </h2>
                        <div class="text-sm text-gray-400">
                            <span class="font-medium text-blue-400">{{ $especialidades->count() }}</span> especialidades registradas
                        </div>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-700">
                            ...
                            @forelse ($especialidades as $esp)
                                <tr class="hover:bg-gray-700 transition-colors duration-150">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-white">{{ $esp->id }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-100 font-medium">{{ $esp->nombre }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-400 max-w-xs truncate">{{ $esp->descripcion }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                    {{ $esp->estado ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300' }}">
                                    {{ $esp->estado ? 'Activo' : 'Inactivo' }}
                                </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end space-x-2">
                                            <!-- Editar -->
                                            <a href="{{ route('especialidades.edit', $esp->id) }}"
                                               class="text-blue-400 hover:text-blue-300 p-1.5 rounded-full hover:bg-gray-700 transition"
                                               title="Editar">
                                                <i class="fas fa-pen fa-sm"></i>
                                            </a>
                                            ...
                                            <form method="POST" action="{{ route('especialidades.toggle', $esp->id) }}">
                                                @csrf
                                                <button type="submit" class="text-yellow-400 hover:text-yellow-300 p-1.5 rounded-full hover:bg-gray-700 transition" title="Cambiar estado">
                                                    <i class="fas fa-toggle-{{ $esp->estado ? 'on' : 'off' }} fa-sm"></i>
                                                </button>
                                            </form>
                                            ...
                                            <form method="POST" action="{{ route('especialidades.destroy', $esp->id) }}"
                                                ...
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-red-400 hover:text-red-300 p-1.5 rounded-full hover:bg-gray-700 transition" title="Eliminar">
                                                    <i class="fas fa-trash fa-sm"></i>
                                                </button>
                                            </form>
                                        </div>
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center text-gray-400 py-4">No hay especialidades registradas.</td>
                                </tr>
                            @endforelse
                            </tbody>
                        </table>
